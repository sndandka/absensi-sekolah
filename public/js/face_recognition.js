...
async function camera() {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      p => {
        geoPosition = { lat: p.coords.latitude, lng: p.coords.longitude };
        const el = id('geoStat');
        if (el) el.innerHTML = '<span class="bdg bg">📍 GPS OK (' + geoPosition.lat.toFixed(4) + ', ' + geoPosition.lng.toFixed(4) + ')</span>';
      },
      () => {
        const el = id('geoStat');
...
async function catatHadir() {
  ...
  try {
    ...
    const maxRadius = parseFloat(sett.maxRadius) || 30; // Default 30m
    ...
    if (!isNaN(schoolLat) && !isNaN(schoolLng)) {
      if (!geoPosition) {
        id('btnCatatHdr').disabled = false;
        id('btnCatatHdr').innerHTML = '🚀 Catat Kehadiran Sekarang';
        return showToast('❌ GPS tidak ditemukan! Aktifkan lokasi dan refresh.', 'error');
      }
      const dist = getDistance(schoolLat, schoolLng, geoPosition.lat, geoPosition.lng);
      if (dist > maxRadius) {
        scanMsg('❌ Anda ' + Math.round(dist) + 'm dari sekolah. Maks ' + maxRadius + 'm.', 'err');
        id('btnCatatHdr').disabled = false;
        id('btnCatatHdr').innerHTML = '🚀 Catat Kehadiran Sekarang';
        return showToast('Gagal: Anda di luar radius ' + maxRadius + 'm dari sekolah!', 'error');
      }
    }
