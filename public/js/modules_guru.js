...
async function jadwal() {
  ...
  // Otomatis pilih hari aktif hari ini jika perannya adalah guru
  const schDaySel = id('schDay');
  if (schDaySel && App.profile?.role === 'guru') {
    schDaySel.value = new Date().getDay();
  }
        ...
        <td>
          <div class="flex gap1">
            ...
            ` : role === 'guru' ? `
              ...
              <button class="btn btn-xs btn-out" onclick="goToInputPresensi(${s.class_id}, ${s.subject_id}, ${s.day}, '${s.start_time}', '${s.end_time}')"><img src="image/add-document.png" style="width:1em;height:1em;vertical-align:middle"> Input Presensi</button>
            ` : ''}
          </div>
        </td>
      </tr>`;
...
function goToInputPresensi(classId, subjectId, day, startTime, endTime) {
  window._absensiPrefill = { classId, subjectId, day, startTime, endTime };
  navigateTo('absensi');
}
